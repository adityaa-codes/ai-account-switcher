...
def test_switch_to_chatgpt_profile_clears_codex_env_var(
    tmp_config_dir: Path, fake_codex_dir: Path, sample_chatgpt_auth_json: dict
) -> None:
    from switcher.utils import get_config_dir

    mgr = CodexProfileManager()
    api_profile = mgr.add_profile("api-personal", "apikey")
    (api_profile.path / "auth.json").write_text(
        json.dumps({"OPENAI_API_KEY": "sk-test-key-abc", "tokens": None}),
        encoding="utf-8",
    )

    chatgpt_profile = mgr.add_profile("chatgpt-personal", "chatgpt")
    (chatgpt_profile.path / "auth.json").write_text(
        json.dumps(sample_chatgpt_auth_json), encoding="utf-8"
    )
    ...
    mgr.switch_to("chatgpt-personal")
