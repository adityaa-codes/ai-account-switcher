...
_MARKER_START = "# >>> ai-account-switcher >>>"
_MARKER_END = "# <<< ai-account-switcher <<<"
_LEGACY_MARKER_START = "# >>> cli-switcher >>>"
_LEGACY_MARKER_END = "# <<< cli-switcher <<<"
...
def inject_into_rc(rc_path: Path) -> bool:
    ...
    if rc_path.exists():
        ...
        if _MARKER_START in content or _LEGACY_MARKER_START in content:
            return False
    else:
        content = ""
...
def remove_from_rc(rc_path: Path) -> bool:
    ...
    for line in lines:
        if _MARKER_START in line or _LEGACY_MARKER_START in line:
            inside = True
            removed = True
            continue
        if _MARKER_END in line or _LEGACY_MARKER_END in line:
            inside = False
            continue
        if not inside:
...
def copy_hook_scripts() -> None:
    """Copy hook .py files to ~/.config/ai-account-switcher/hooks/."""
