# AI Account Switcher — Technical Specification
...
- XDG Base Directory compliance (`~/.config/ai-account-switcher/`)
...
| 8 | **No XDG compliance** | Dumps files in `~/.gemini/` alongside CLI's own files | `~/.config/ai-account-switcher/` for all tool state |
...
```
$XDG_CONFIG_HOME/ai-account-switcher/           # defaults to ~/.config/ai-account-switcher/
...
```bash
# Auto-generated by ai-account-switcher — do not edit manually
...
1. Read API key from profiles/gemini/<label>/api_key.txt
   (or from keyring: service=ai-account-switcher-gemini, key=<label>)
...
- ai-account-switcher-gemini / <label>        → Gemini API keys (our own)
- ai-account-switcher-codex / <label>         → Codex API keys (our own)
```
  ...
  "hooks": {
    "AfterAgent": [{
      ...
      "hooks": [{
        ...
        "type": "command",
        "command": "python3 /home/<user>/.config/ai-account-switcher/hooks/gemini_after_agent.py",
        "timeout": 10000,
    ...
    "BeforeAgent": [{
      ...
      "hooks": [{
        ...
        "type": "command",
        "command": "python3 /home/<user>/.config/ai-account-switcher/hooks/gemini_before_agent.py",
        "timeout": 10000,
...
2. **Create env.sh** at `~/.config/ai-account-switcher/env.sh`
3. **Inject into shell RC** (`~/.bashrc` or `~/.zshrc`):

```bash
# --- AI Account Switcher (auto-generated) ---
[ -f "$HOME/.config/ai-account-switcher/env.sh" ] && source "$HOME/.config/ai-account-switcher/env.sh"
alias sw='switcher'
# Shell wrappers to source env before CLI launch
gemini() { source "$HOME/.config/ai-account-switcher/env.sh" 2>/dev/null; command gemini "$@"; }
codex() { source "$HOME/.config/ai-account-switcher/env.sh" 2>/dev/null; command codex "$@"; }
# --- End AI Account Switcher ---
```
...
- Log to `~/.config/ai-account-switcher/logs/switcher.log`
...
- [x] **T-005** Implement `switcher/utils.py`:
  - [x] T-005a: `get_config_dir()` — resolve `$XDG_CONFIG_HOME/ai-account-switcher` with fallback to `~/.config/ai-account-switcher`
...
- [x] **T-020** Implement `switcher/installer.py`:
  ...
  - [x] T-020m: `copy_hook_scripts()` — copy hook .py files to `~/.config/ai-account-switcher/hooks/`
